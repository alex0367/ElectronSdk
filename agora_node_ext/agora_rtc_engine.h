        ...
        */
        ...
        public:
            ...
            */
            ...
            NAPI_API(addPublishUrl);
            NAPI_API(removePublishUrl);

            /* New RTC API included in 2.1, still not implement yet.
            ...
            /* Wrapper for IRtcEngineEx, internal, not implement right now.
            NAPI_API(initializeEx)
            NAPI_API(setVideoCanvas)
            NAPI_API(setParameters)
            NAPI_API(getParameters)
            NAPI_API(setProfile)
            NAPI_API(getProfile)
            NAPI_API(notifyNetworkChange)
            NAPI_API(sendReportMessage)
            NAPI_API(getOptionsByVideoProfile)
            NAPI_API(setLogCallback)
            NAPI_API(makeQualityReportUrl)
            NAPI_API(reportWebAgentVideoStats)
            NAPI_API(printLog)
            NAPI_API(runOnWorkerThread)
            NAPI_API(reportArgusCounters)
            NAPI_API(setVideoCompositingLayout2)
            NAPI(API(startScreenCaptureEx)
            NAPI_API(addVideoCapture)
            NAPI_API(addLocalVideoRender)
            NAPI_API(addVideoReceiveTrack)
            NAPI_API(removeVideoReceiveTrack)
            NAPI_API(addInjectStreamUrl2)

            NAPI_API(setVideoProfileEx)
            */
            ...
            */
            ...
            NAPI_API(setPlaybackDeviceVolume);
            ...
            */
            ...
            NAPI_API(setAudioPalybackDeviceMute);
        ...
        protected:
            ...
            /**
             * To find video channel with specified uid
             * @param uid - the channel's uid
             */
            NodeVideoStreamChannel* findChannelByUid(uid_t uid);

            /**
             * To remove the channel with the specified uid
             * @param uid - the channel's uid
             */
            bool removeChannelByUid(uid_t uid);
        private:
            DECLARE_CLASS;
            IRtcEngine *m_engine;
            NodeEventHandler *m_eventHandler;
            Isolate *m_isolate;
            IExternalVideoRenderFactory *m_externalVideoRenderFactory;
            std::list<std::shared_ptr<NodeVideoStreamChannel>> m_channels;
...
#define NAPI_API_DEFINE_WRAPPER_PARAM_0(method) \
        ...
        { \
            ...
            do {\
                ...
                int result = pEngine->m_engine->##method();\
